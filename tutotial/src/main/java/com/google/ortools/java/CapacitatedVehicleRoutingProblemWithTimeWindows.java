...
public class CapacitatedVehicleRoutingProblemWithTimeWindows {
  ...
  private void buildOrders(int numberOfOrders, int xMax, int yMax, int demandMax, int timeWindowMax,
      int timeWindowWidth, int penaltyMin, int penaltyMax , List<String> stngFile) {
    logger.info("Building orders.");
    for (int order = 0; order < numberOfOrders; ++order) {
      locations.add(Pair.of(
    		  Integer.valueOf(stngFile.get( stngFile.indexOf("SERVICETIME")+2 + 8*order)),
    		  Integer.valueOf(stngFile.get( stngFile.indexOf("SERVICETIME")+3 + 8*order)))
    		  );
  
      orderDemands.add( Integer.valueOf(stngFile.get( stngFile.indexOf("DEMAND")+2 + 2*order)) );
    
      int timeWindowStart = 0;
      orderTimeWindows.add(Pair.of(
    		  Integer.valueOf(stngFile.get( stngFile.indexOf("SERVICETIME")+5 + 8*order)), 
    		  Integer.valueOf(stngFile.get( stngFile.indexOf("SERVICETIME")+6 + 8*order)))
    		  );
      orderPenalties.add(2000000);
    }
  }
  ...
  private void buildFleet(
      int numberOfVehicles, int xMax, int yMax, int endTime, int capacity, int costCoefficientMax, List<String> stngFile) {
  ...
  public static void main(String[] args) throws Exception {
    ...
    Scanner scnr = new Scanner(new FileReader(
            "C:\\Users\\malla\\Downloads\\VRPTW_Data\\V_50_5.txt"));
    String str;
    while (scnr.hasNext()) {
                str = scnr.next();
                stngFile.add(str);
    ...
    final int orders = Integer.valueOf(stngFile.get(stngFile.indexOf("NUMTYPS")+3));
    final int vehicles = Integer.valueOf(stngFile.get(stngFile.indexOf("NUMTYPS")+1));
    final int capacity = Integer.valueOf(stngFile.get(stngFile.indexOf("NUMTYPS")+2));

    problem.buildOrders(
        orders, xMax, yMax, demandMax, timeWindowMax, timeWindowWidth, penaltyMin, penaltyMax , stngFile);
    problem.buildFleet(vehicles, xMax, yMax, endTime, capacity, costCoefficientMax, stngFile);
